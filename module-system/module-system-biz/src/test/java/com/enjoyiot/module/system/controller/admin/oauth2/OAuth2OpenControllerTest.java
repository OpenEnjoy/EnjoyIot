...
public class OAuth2OpenControllerTest extends BaseMockitoUnitTest {
    ...
    public void testApproveOrDeny_ApproveNo() {
        ...
        String clientId = randomString();
        String scope = "{\"read\": true, \"write\": false}";
        String redirectUri = "http://www.enjoy-iot.cn";
        String state = "test";
        // mock 方法
        OAuth2ClientDO client = randomPojo(OAuth2ClientDO.class);
        ...
        // 断言
        assertEquals(0, result.getCode());
        assertEquals("http://www.enjoy-iot.cn#error=access_denied&error_description=User%20denied%20access&state=test", result.getData());
    }
    ...
    public void testApproveOrDeny_autoApproveWithToken() {
        ...
        assertThat(result.getData(), anyOf( // 29 和 30 都有一定概率，主要是时间计算
                is("http://www.enjoy-iot.cn#access_token=test_access_token&token_type=bearer&state=test&expires_in=29&scope=read"),
                is("http://www.enjoy-iot.cn#access_token=test_access_token&token_type=bearer&state=test&expires_in=30&scope=read")
        ));
    }
    ...
    public void testApproveOrDeny_approveWithCode() {
        ...
        // 断言
        assertEquals(0, result.getCode());
        assertEquals("http://www.enjoy-iot.cn?code=test_code&state=test", result.getData());
    }
