...
import static com.enjoyiot.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("ENJOY", user.getUsername());
            assertEquals("test", user.getPassword());
            assertEquals("test", user.getNickname());
            assertEquals("1888888888", user.getMobile());
            return true;
        }))).thenReturn(300L);

        // 准备参数
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setContactName("test");
            o.setContactMobile("15601691300");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setWebsite("http://www.enjoy-iot.cn");
            o.setUsername("ENJOY");
            o.setPassword("enjoyiot");
    ...
    public void testGetTenantPage() {
        ...
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> { // 等会查询到
            o.setName("EnjoyIot");
            o.setContactName("ENJOY");
            o.setContactMobile("1888888888");
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("test"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("test");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    public void testGetTenantByWebsite() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setWebsite("http://www.enjoy-iot.cn"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByWebsite("http://www.enjoy-iot.cn");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
