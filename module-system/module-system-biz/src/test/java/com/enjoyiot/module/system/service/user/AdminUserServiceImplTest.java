...
import static com.enjoyiot.module.system.service.user.AdminUserServiceImpl.USER_INIT_PASSWORD_KEY;
...
public class AdminUserServiceImplTest extends BaseDbUnitTest {
    ...
    public void before() {
        // mock 初始化密码
        when(configApi.getConfigValueByKey(USER_INIT_PASSWORD_KEY)).thenReturn("enjoyiot");
    }
    ...
    public void testCreatUser_success() {
        ...
        when(passwordEncoder.encode(eq(reqVO.getPassword()))).thenReturn("enjoy-iot");
        ...
        assertEquals("enjoy-iot", user.getPassword());
    ...
    public void testImportUserList_02() {
        ...
        when(deptService.getDept(eq(dept.getId()))).thenReturn(dept);
        // mock passwordEncoder 的方法
        when(passwordEncoder.encode(eq("test"))).thenReturn("java");
    ...
    public void testGetUserListByNickname() {
        // mock 数据
        AdminUserDO user = randomAdminUserDO(o -> o.setNickname("ENJOY-IOT"));
        userMapper.insert(user);
        // 测试 nickname 不匹配
